...
class HardwareProtectionLayer(AbstractPattern):
    ...
    def __init__(self):
        """Initialize the node."""
        super().__init__('hardware_protection_layer_TEST')
        ...
        self.get_logger().info(f"Max range: {self.param_max_range}\nMin range: {self.param_min_range}\nThreshold: {self.param_threshold}\nMax vel: {self.param_max_translational_velocity}")
    ...
    def vector_calc(self):
        ...
        # self.get_logger().info("HELLO3")

        if self.current_ranges is None:
            self.get_logger().info("No range data!!!!")
            return [False, None]
        ...
        self.get_logger().info(f"Avoid Needed: {avoid_needed}")
