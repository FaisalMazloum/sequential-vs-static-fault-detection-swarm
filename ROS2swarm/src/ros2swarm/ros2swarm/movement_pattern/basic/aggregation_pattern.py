...
class AggregationPattern(MovementPattern):
    ...
    # MY CUSTOM FUNCTION
    # =======================================================================
    def identify_obstacles(self, msg):
        """Identifies if a scan contains objects, based on the settings in the pattern parameters.

        :return: return the list of all found objects and a list in which each object is represented by a single ray
         based on the chosen reduction method
        """
        objects, objects_center = ScanCalculationFunctions.identify_obstacles(range_data=msg,
                                                                         min_range=self.param_min_range,
                                                                         max_range=self.param_max_range,
                                                                         threshold=self.param_object_threshold,
                                                                         reduction=self.param_reduction)
        self.get_logger().debug('Found "{}" objects'.format(len(objects)))
    ...
    #     -> EXPLORE : else """
    #     robots, _ = self.identify_robots(range_data)
    #     _, objects_center = self.identify_obstacles(range_data)
    #     result = Twist()
    #     if robots:
    #         # As log as robots are within detection range move way from all objects (robot + obstacles)
    #         state = State.LEAVE_GROUP
    #         result = ScanCalculationFunctions.create_twist_from_objects(
    #             centered_object_list=objects_center,
    #             max_rotational_velocity=self.param_max_rotational_velocity,
    #             max_translational_velocity=self.param_max_translational_velocity,
    #             max_range=self.param_max_range,
    #             front_attraction=self.param_front_attraction,
