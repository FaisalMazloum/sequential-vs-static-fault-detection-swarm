...
class ScanCalculationFunctions:
    ...
    #     print(f'vector[0] = {float(vector[0])}, vector[1] = {float(vector[1])}')
    ...
    def create_normed_twist_message(vector, max_translational_velocity, max_rotational_velocity):
        ...
        # Angular velocity - fixed with atan2
        angle = math.atan2(float(vector[1]), float(vector[0]))
        
        # Deadzone: stop rotating if close to perpendicular
        # print(f'angle = {angle}')
        # DEADZONE_ANGLE = 7.0   # deadzone
        # if abs(angle) < DEADZONE_ANGLE:
            # direction.angular.z = 0.0
        # else:
        angle_normalized = np.clip(angle / (math.pi / 2), -1.0, 1.0)
        direction.angular.z = float(max_rotational_velocity * angle_normalized)
    ...
    # =========================================================================
    @staticmethod
    def identify_obstacles(range_data: RangeData, min_range: float, max_range: float, threshold: float,
                        reduction: ReductionOption = ReductionOption.NEAREST):
        """Identify robots from scan within given range. Objects are defined by rays that are beside each other and
        within the threshold of the last ray of the current object. An object is identified as robots if the object size
        is between min_width and max_width.
        :param laser_scan: scan of the environment
        ...
        if reduction is ReductionOption.NEAREST:
            objects_center = ScanCalculationFunctions.reduce_objects_to_nearest(objects)
        elif reduction is ReductionOption.MEAN:
            objects_center = ScanCalculationFunctions.reduce_objects_to_mean(objects)
        else:
            raise RuntimeError('Reduction behaviour not supported.')

        return objects, objects_center
    ...
    def create_twist_for_objects(centered_object_list: list,
        ...
        if select_target is ReductionOption.NEAREST:
            ...
            objects = centered_objec#    Copyright 2020 Marian Begemann
#
#    Licensed under the Apache License, Version 2.0 (the "License");
#    you may not use this file except in compliance with the License.
#    You may obtain a copy of the License at
...
class ScanCalculationFunctions:
    ...
    def create_normed_twist_message(vector, max_translational_velocity, max_rotational_velocity):
        ...
        # Linear velocity - allows backward motion
        direction.linear.x = float(max_translational_velocity * vector[0])
    ...
    # =========================================================================
    @staticmethod
    def identify_object(range_data: RangeData, min_range: float, max_range: float, threshold: float,
                        reduction: ReductionOption = ReductionOption.NEAREST):
        """Identify robots from scan within given range. Objects are defined by rays that are beside each other and
        within the threshold of the last ray of the current object.
        :param laser_scan: scan of the environment
        :param min_range: minimum length of a ray to consider it
        :param max_range: maximum length of a ray to consider it
        :param threshold: Length difference between two rays to map them to the same robot
